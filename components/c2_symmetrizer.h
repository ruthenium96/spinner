...
class Symmetrizer {
public:
    Symmetrizer(std::vector<int> mults_, int pairs_) : mults(mults_), pairs(pairs_) {
        tensor_size = 1;
        for (int mult : mults) {
            tensor_size *= mult;
        }
    ...
    std::vector<std::map<Index, Coefficient>> projector(std::map<Index, Coefficient> & m,
    ...
    static void erase_if_zero(std::vector<std::map<Index, Coefficient>> & projections) {
