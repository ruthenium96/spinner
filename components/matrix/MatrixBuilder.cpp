...
MatrixBuilder::MatrixBuilder(lexicographic::IndexConverter converter) :
...
Submatrix
MatrixBuilder::apply_to_subentity(const Subspace& subspace, const Operator& new_operator) {
        ...
        while (outer_iterator->hasNext()) {
            ...
            if (built_lexicografical_vectors.count(index_of_lexicographic_vector_k) == 0) {
                ...
                for (int center_a = 0; center_a < converter_.get_mults().size(); ++center_a) {
                    ...
                    for (int center_b = center_a + 1; center_b < converter_.get_mults().size();
                         ++center_b) {
            ...
            for (uint32_t index_of_space_vector_j = 0;
                ...
                while (inner_iterator->hasNext()) {
                    ...
                    if (value_in_matrix_in_lexicografical_basis != 0) {
                        submatrix.raw_data.add_to_position(
