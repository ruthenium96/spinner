...
namespace {
bool SizeOfPermutationsEqualsNumberOfSpins (const lexicographic::IndexConverter& converter,
                                            const Group& group) {
    return converter.get_mults().size() == group.elements_[0].size();
}

bool OrbitOfCentersHasTheSameValueOfMultiplicity (const lexicographic::IndexConverter& converter,
                                                const Group& group) {
...
Symmetrizer::Symmetrizer(lexicographic::IndexConverter converter, Group group)
    ...
    for (size_t i = 0; i < space.blocks.size(); ++i) {
        ...
        for (uint32_t l = 0; l < subspace_parent.decomposition.size(); ++l) {
            ...
            if (count_how_many_orbit_was_visited(subspace_parent.decomposition, l, visited) < dimension_of_parent) {
                std::vector<UnitarySparseMatrix> projected_basi = get_symmetrical_projected_decompositions(subspace_parent, l);
...
std::vector<UnitarySparseMatrix> Symmetrizer::get_symmetrical_projected_decompositions(Subspace& subspace,
...
void Symmetrizer::increment_visited(const UnitarySparseMatrix& decomposition,
...
uint8_t Symmetrizer::count_how_many_orbit_was_visited(const UnitarySparseMatrix& decomposition,
                                                      uint32_t index_of_vector,
                                                      std::unordered_map<uint32_t , uint8_t>& hs) {
    uint8_t maximum = 0;
    auto iterator = decomposition.GetNewIterator(index_of_vector);
...
bool Symmetrizer::is_orthogonal_to_others(const UnitarySparseMatrix& decomposition_from, uint32_t index_of_vector,
...
void Symmetrizer::move_vector_and_remember_it(UnitarySparseMatrix& decomposition_from, uint32_t index_of_vector,
                                              std::unordered_map<uint32_t, std::vector<size_t>>& hs,
