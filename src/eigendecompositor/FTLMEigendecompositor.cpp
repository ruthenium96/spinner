...
FTLMEigendecompositor::BuildSubspectra(
    ...
    std::optional<std::vector<std::shared_ptr<linalg_structures::AbstractDenseSemiunitaryMatrix>>>
    ...
    if (!do_we_need_eigenvectors_) {
        std::vector<std::unique_ptr<linalg_structures::AbstractDenseVector>> weights_of_all_seeds_;
...
#pragma omp taskloop default(shared)
        ...
        mb_unitary_transformation_matrix = 
            std::vector<std::shared_ptr<linalg_structures::AbstractDenseSemiunitaryMatrix>>(number_of_seeds_);
...
FTLMEigendecompositor::getWeightsOfBlockStates(size_t number_of_block) const {
    ...
    if (getOneRef(exact_weights).get() != nullptr) {
        ...
        std::vector<std::reference_wrapper<const std::unique_ptr<linalg_structures::AbstractDenseVector>>> answer;
