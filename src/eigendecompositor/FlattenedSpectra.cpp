...
spinner::OneOrMany<std::unique_ptr<spinner::linalg_structures::AbstractDenseVector>> flatten(
    spinner::OneOrMany<spinner::SpectrumRef> spectrum,
    const spinner::linalg_structures::FactoriesList& factories) {
...
void FlattenedSpectra::updateValues(const AllQuantitiesGetter& allQuantitiesGetter,
    ...
    flattenedWeights_ = transform_one_or_many(
        std::function([factories](std::vector<std::reference_wrapper<const std::unique_ptr<linalg_structures::AbstractDenseVector>>> weights_all){
            auto vector = factories.createVector();
    ...
    for (const auto& [quantity_enum, spectrum] : flattenedSpectra_) {
        ...
        for (const auto& [derivative_key, spectrum_derivative] : flattenedDerivativeSpectra_) {
            ...
            flattenedDerivativeProductSpectra_[derivative_product_key] = 
                ...
                std::function([](
                    const std::unique_ptr<linalg_structures::AbstractDenseVector>& a, 
                    const std::unique_ptr<linalg_structures::AbstractDenseVector>& b){
...
FlattenedSpectra::getFlattenSpectrum(common::QuantityEnum quantity_enum) const {
    if (flattenedSpectra_.contains(quantity_enum)) {
        ...
        return copyRef<std::unique_ptr<linalg_structures::AbstractDenseVector>, 
            std::reference_wrapper<const std::unique_ptr<linalg_structures::AbstractDenseVector>>>(spectrum);
    } else {
        return std::nullopt;
    }
}
...
OneOrMany<std::reference_wrapper<const std::unique_ptr<linalg_structures::AbstractDenseVector>>> 
    ...
    return copyRef<std::unique_ptr<linalg_structures::AbstractDenseVector>, 
        std::reference_wrapper<const std::unique_ptr<linalg_structures::AbstractDenseVector>>>(flattenedWeights_);
}
