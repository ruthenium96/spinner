...
using namespace spinner::model;

TEST(symbolic_worker, throw_assign_the_empty_name) {
    symbols::SymbolicWorker symbols(2);
...
TEST(symbolic_worker, throw_2222_gfactor_only_one_g_was_initialized) {
    ...
    EXPECT_THROW(spinner::runner::Runner runner(modelInput), std::invalid_argument);
}
...
TEST(symbolic_worker, throw_set_new_value_to_unchangeable_symbol) {
    symbols::SymbolicWorker symbols_(10);
...
TEST(symbolic_worker, throw_specified_not_as_J_symbol) {
    ...
    symbols::SymbolicWorker symbols_(number_of_spins);
...
TEST(numerical_worker, set_new_value_to_changeable_J_g) {
    ...
    auto J_changeable =
        symbols_.addSymbol("JChangeable", JChangeable_value, true, symbols::J);
    auto g_changeable =
    ...
    auto parametersHandler = symbols::NumericalWorker(symbols_, number_of_spins);
