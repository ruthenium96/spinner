...
TEST(hamiltonian_operator, exchange_interaction_22_333_4444_23456) {
    ...
    for (const auto& mults : vector_of_mults) {
        spinner::model::ModelInput modelInput(mults);

        // explicitly initialize isotropic exchange:
        auto J = modelInput.addSymbol("J", 10);
        modelInput.assignSymbolToIsotropicExchange(J, 0, 1);

        spinner::runner::ConsistentModelOptimizationList model_and_optimization(modelInput, spinner::common::physical_optimization::OptimizationList());

        EXPECT_EQ(model_and_optimization.getModel().getOperator(spinner::common::QuantityEnum::Energy).value()->getTerms().size(), 1);
    }
}
