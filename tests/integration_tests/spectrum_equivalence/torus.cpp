...
void initialize_nine_centers_torus(spinner::model::ModelInput& model, double first) {
...
#define group_torus_hor Group({Dihedral, 3}, {{3, 4, 5, 6, 7, 8, 0, 1, 2}, {0, 1, 2, 6, 7, 8, 3, 4, 5}})
#define group_torus_ver Group({Dihedral, 3}, {{1, 2, 0, 4, 5, 3, 7, 8, 6}, {0, 2, 1, 3, 5, 4, 6, 8, 7}})

#define group_torus_hor_S2 Group(S2, {{0, 1, 2, 6, 7, 8, 3, 4, 5}})
#define group_torus_ver_S2 Group(S2, {{0, 2, 1, 3, 5, 4, 6, 8, 7}})
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        std::vector<spinner::spin_algebra::Multiplicity>({2, 2, 2, 2, 2, 2, 2, 2, 2})
    )
),
