...
void initialize_prism_system(spinner::model::ModelInput& model, double J_first_value, double J_second_value) {
...
Group generate_Dn_group_for_prism(size_t size) {
    Permutation generator_rotation(2 * size);
    for (int i = 0; i < size; ++i) {
        generator_rotation[i] = (i + 1) % size;
        generator_rotation[i + size] = (i + 1) % size + size; 
    }
    Permutation generator_reflection(2 * size);
    for (int i = 0; i < size; ++i) {
...
Group generate_C2_mirror_group_for_prism(size_t size) {
    Permutation generator_mirror(2 * size);
    for (int i = 0; i < size; ++i) {
...
Group generate_first_C2_group_for_prysm(size_t size) {
    Permutation generator_reflection(2 * size);
    for (int i = 0; i < size; ++i) {
        generator_reflection[size - 1 - i] = i;
        generator_reflection[2 * size - 1 - i] = i + size;
