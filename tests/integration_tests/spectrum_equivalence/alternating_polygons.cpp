...
void initialize_alternating_polygon_system(spinner::model::ModelInput& model, double J_first_value, double J_second_value) {
...
Group generate_Dn_half_group_for_alternating_polygon(size_t size) {
    ...
    return Group({Group::Dihedral, size / 2},
        {generator_rotation, generator_reflection});
}

Group generate_first_C2_group_for_alternating_polygon(size_t size) {
    Permutation generator_reflection(size);
    for (int i = 0; i < size; ++i) {
        generator_reflection[size - 1 - i] = i;
    }
...
Group generate_second_C2_group_for_alternating_polygon(size_t size) {
    if (size % 4 != 0) {
