...
void expect_mu_squared_approximate_equality(spinner::runner::Runner& exact, spinner::runner::Runner& ftlm) {
...
TEST(ftlm_integration_tests, 10x2_FM_ring_SameG) {
    std::vector<spinner::spin_algebra::Multiplicity> mults = {2, 2, 2, 2, 2, 2, 2, 2, 2, 2};
    ...
    spinner::common::physical_optimization::OptimizationList exact_optimization_list;
    exact_optimization_list.TzSort().EliminatePositiveProjections();
    spinner::runner::Runner exact_runner(model, exact_optimization_list);

    spinner::common::physical_optimization::OptimizationList ftlm_optimization_list;
    ftlm_optimization_list.TzSort().EliminatePositiveProjections().FTLMApproximate({100, 128, 100});
    spinner::runner::Runner ftlm_runner(model, ftlm_optimization_list);
    
    expect_mu_squared_approximate_equality(exact_runner, ftlm_runner);
}

TEST(ftlm_integration_tests, 12x2_FM_ring_SameG) {
    std::vector<spinner::spin_algebra::Multiplicity> mults = {2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2};
    spinner::model::ModelInput model(mults);
    auto g = model.addSymbol("g", 2.0);
    auto J = model.addSymbol("J", +10.0);
    for (int center = 0; center < mults.size(); ++center) {
...
TEST(ftlm_integration_tests, 14x2_FM_ring_SameG) {
    std::vector<spinner::spin_algebra::Multiplicity> mults = {2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2};
    spinner::model::ModelInput model(mults);
    auto g = model.addSymbol("g", 2.0);
    auto J = model.addSymbol("J", +10.0);
    for (int center = 0; center < mults.size(); ++center) {
