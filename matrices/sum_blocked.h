#ifndef JULY_SUM_BLOCKED_H
#define JULY_SUM_BLOCKED_H
...
struct Proj_Blocked_Matrices : Matrices {
    ...
    Add_To_Matrix s_squared_ = static_cast<Add_To_Matrix>(&Proj_Blocked_Matrices::add_to_s_squared);
    ...
    Add_To_Matrix hamiltonian_ = static_cast<Add_To_Matrix>(&Proj_Blocked_Matrices::add_to_hamiltonian);

    Proj_Blocked_Matrices(const Indexes_P2 &blm_,
    ...
    void eigendecomposition(arma::vec & eigval, arma::vec & s_squared_new_basis_vector,
        ...
        for (unsigned int r = 0; r < blm.num_of_repr; ++r) {
            for (int i = 0; i < blm.sym_sum_boundaries[r].size() - 1; ++i) {
                current_start = blm.sym_sum_boundaries[r][i];
                current_end = blm.sym_sum_boundaries[r][i + 1];
