...
//    }
    ...
    void scalar_product_total(Add_To_Matrix add_to_matrix, unsigned int repr, const double factor,
        ...
        for (const Decomposition & d : blm.sym2block[sym]) {
            ...
            double decomposition_factor = d.coeff * blm.block2sym[d.index][repr].coeff;
...
private:
    void scalar_product_plus_minus(Add_To_Matrix add_to_matrix, unsigned int repr, const double factor,
        ...
        if (repr < blm.block2sym[new_block].size()) {
            unsigned long new_sym = blm.block2sym[new_block][repr].index;
            double decomposition_factor = d.coeff * blm.block2sym[new_block][repr].coeff;
