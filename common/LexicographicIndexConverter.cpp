...
uint8_t spaces::LexicographicIndexConverter::lex_to_ntzproj(uint32_t lex) const {
    uint8_t ntz_proj = 0;
...
std::vector<uint8_t> spaces::LexicographicIndexConverter::lex_to_nzs(uint32_t lex) const {
...
uint32_t spaces::LexicographicIndexConverter::nzs_to_lex(const std::vector<uint8_t> &nzs) const {
    uint32_t lex = 0;
    for (int i = 0; i < mults_.size(); ++i) {
