...
void SparseMatrix::add_scalar_product(uint32_t index_of_vector, uint32_t center_a, uint32_t center_b, double factor) {
    ...
    add_scalar_product_nondiagonal_part(index_of_vector, center_a, center_b,
                                        projection_of_center_a, projection_of_center_b, factor);
    // Sa- Sb+
    add_scalar_product_nondiagonal_part(index_of_vector, center_b, center_a,
...
void SparseMatrix::add_scalar_product_nondiagonal_part(uint32_t index_of_vector, uint32_t plus_center, uint32_t minus_center,
    ...
    uint32_t index_of_new_vector = converter_.ladder_projection(
            converter_.ladder_projection(
                    index_of_vector, plus_center, +1),
                    minus_center, -1
                    );
    ...
    double factor_a = (2 * converter_.get_spins()[plus_center] - projection_of_plus_center) * (projection_of_plus_center + 1);
    double factor_b = projection_of_minus_center * (2 * converter_.get_spins()[minus_center] + 1 - projection_of_minus_center);
